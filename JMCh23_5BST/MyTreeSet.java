...
*/
    ...
    {
        TreeNode<E> nodeR = root.getRight();
        TreeNode<E> nodeL = root.getLeft();
        ...
        {
            root.setValue( nodeR.getValue() );
            root.setRight( nodeR.getRight());
            return root;
        }
        TreeNode<E> prev = null;
        
        while (nodeR.getLeft() != null)
        {
            prev = nodeR;
            nodeR = nodeR.getLeft();
        }
        ...
        root.setValue( nodeR.getValue() );
        root = new TreeNode<E>(nodeR.getValue(), root.getLeft(), root.getRight());
        TreeNode<E> nodeRR = nodeR.getRight();
        
        if (nodeRR != null)
        {
            nodeR = nodeRR;
            nodeR.setRight( nodeRR.getRight() );
            nodeR.setLeft( nodeRR.getLeft() );
            prev.setLeft( nodeR );
        }
        else
        {
    ...
    {
        Stack<TreeNode<E>> stackTree;
        TreeNode<E> tNode;

        public MyTreeSetIterator( TreeNode<E> root )
        {
            stackTree = new Stack<TreeNode<E>>();
            tNode = root;
            
            while (tNode != null)
            {
                TreeNode<E> temp = tNode;
                stackTree.push( temp );
                tNode = tNode.getLeft();
            }
        }
