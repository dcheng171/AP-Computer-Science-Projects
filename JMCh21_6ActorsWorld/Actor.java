 ...
 * 
 * @author Period - 4
 * @author Assignment - Actor
 * 
 * @author Sources - given
 */
public class Actor implements Comparable<Actor>
{
    ...
    public void readMail()
    {
        ...
        {
            Message x = mailbox.remove();
            for (String item : items)
            {
                int count = 0;
                if (x.getText().equals("need " + item))
                {
                    for (String e : myPossessions)
                    {
                        if (e.equals( item ))
                        {
                            count++;
                    ...
                    if (count > 1)
                    {
                        reply(x.getSender(), "have " + item);
                    }
                }
                
                else if (x.getText().equals("have " + item))
                {
                    for (String e : myPossessions)
                    {
                        if (e.equals( item ))
                    ...
                    if (count == 0)
                    {
                        reply(x.getSender(), "ship " + item);
                    }
                }
                
                else if (x.getText().equals("ship " + item))
                {
                    for (String e : myPossessions)
                    {
                        if (e.equals( item ))
                    ...
                    if (count > 1)
                    {
                        if(x.getSender().receiveItem( x.getRecipient(), item))
                        {
